...
export default function useParticipants() {
  ...
  const [participants, setParticipants] = useState<Array<ParticipantWithSlot>>(
    Array.from(room?.participants.values() ?? []).map((x, idx) => ({
      participant: x,
  ...
  useEffect(() => {
    if (room) {
      ...
      const participantConnected = (participant: RemoteParticipant) =>
        setParticipants(prevParticipants => {
          let idx = 0;
          const ps = prevParticipants.sort((x, y) =>
            x.slot < y.slot ? -1 : x.slot === y.slot ? 0 : 1,
          );
          while (idx < ps.length) {
